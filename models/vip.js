...
module.exports.person = function(data,callback) {
    db.getConnection(function(err, connexion) {
        if (!err) {
            let sql = "SELECT VIP_NOM AS NAME,VIP_PRENOM AS FIRSTNAME, VIP_NAISSANCE AS DATE, PHOTO_ADRESSE AS PHOTO FROM vip v JOIN photo p ON v.VIP_NUMERO=p.VIP_NUMERO WHERE v.VIP_NOM='"+data+"' AND PHOTO_NUMERO=1;";
            connexion.query(sql, callback);
            connexion.release();
        }
    });
